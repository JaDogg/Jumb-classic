...
Dim Shared As String c_sub_name,c_let_var_i,p_txt
Dim Shared As Integer c_sub_curr_params,p_typ,_asm_code_blk,n_strz,strz_beginm,strz_endm
...
Sub push_var
	...
	If c_typ = c_type_str Then
		...
		Print #f_out, Str ( "PUSH _JUMB_TMP_STR" & cnt_strz )

	ElseIf c_typ = c_type_num Then
		'just move it
		Print #f_out,Str ( "PUSH " & c_txt )
	Else
		If declared_search = 1 Then
			'registered item
			If mov_eax_a_val = 0 Then
...
End Sub
Sub do_let_i()
	'LETINT A (((1+B)*200)+1)*5 xor mod and or
	'c_let_var_i
	...
	Do
		...
		ElseIf c_txt = "/" Then
			...
			Print #f_out,"invoke _jumb_dev"

		ElseIf c_txt = "MOD" Then
			...
			Print #f_out,"invoke _jumb_mod"

		ElseIf c_txt = "AND" Then
			...
			Print #f_out,"invoke _jumb_and"

		ElseIf c_txt = "OR" Then
			...
			Print #f_out,"invoke _jumb_or"

		ElseIf c_txt = "XOR" Then
	...
	Print #f_out, "MOV [" & c_let_var_i & "],EAX"
...
Sub jumbeval_1
	...
	Select Case c_txt
		...
		Case key_words(0)
			...
			Print #f_out,Str("jmp _jumb_defstr_skp" &  cnt_def_intz)
			strz_beginm = n_strz
			...
			Do
				...
				If c_txt <> "" Then
					Print #f_out,Str(c_txt & " dd " & n_strz )
				Else
					Print #f_out,Str("_jumb_defstr_skp" & cnt_def_intz & ":")
					' x 1 10
					Print #f_out,Str("invoke _jumb_add_the_strings," & strz_beginm & "," & n_strz)
					Exit Do
				EndIf
			Loop
		...
		Case "OKMSG"
			...
			If push_method = 1 Then
				mov_to_eax
				j_pushm_code = Str("invoke _jumb_msgbox"  & Chr(13,10) &  j_pushm_code)
				jumbeval_1
...
Do
	...
	If _asm_code_blk  = 1 Then
		If c_txt <> "ASMCODE" Then
			Print #f_out, o_txt
		EndIf
	EndIf
