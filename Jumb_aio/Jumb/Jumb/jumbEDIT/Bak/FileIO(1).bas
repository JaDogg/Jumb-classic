...
Function OpenAFile(ByVal hWin As HWND) As Integer
	...
	ofn.lpstrFilter = StrPtr(!"Jumb Code Files (*.jum)\0*.jum\0\0" )
	ofn.hwndOwner=hWin
	ofn.hInstance=hInstance
	ofn.lpstrInitialDir=StrPtr(EXEFILEPATHX)
	buff=String(260,0)
	ofn.lpstrFile=@buff
	ofn.nMaxFile=260
...
End Sub
Function Want_it_console(ByVal hWin As HWND) As Integer
	Dim x As Integer
	

		Select Case  MessageBox(hWin,"Compile as Console APP ?",AppName,MB_YESNOCANCEL Or MB_ICONQUESTION)
			Case IDYES
				Return 1
				'
			Case IDCANCEL
